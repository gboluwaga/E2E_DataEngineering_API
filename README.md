...
Three DAGs exist, triggered one after the other. These can be accessed using the Airflow UI. The DAGs are as follows;

* *youtube_video_statistics_dag* - DAG to produce JSON file with raw data
* *update_db* - DAG to process JSON file and insert data into booth staging and core schemas
* *data_quality_checks* - DAG to check the data quality on both layers in the database
